...
for density, size in ANDROID_ADAPTIVE.items():
    fore = resize_centered(size, pad_ratio=0.18)
...
# Generate splash_icon.png (icon only, transparent bg, centered with generous padding)
# Android 12+ splash masks icons into a ~72dp circle within 108dp container.
# Logo must fit within ~66% of the canvas to avoid clipping.
for density, size in ANDROID_LAUNCHER.items():
    # Android 12+ splash masks icons into a shape (circle on Samsung).
    # The splash safe-area is ~66% of container. At 20% padding, the logo
    # occupies ~60% of height, fitting safely within the circular mask.
    icon = resize_centered(size, pad_ratio=0.20)
